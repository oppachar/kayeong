...
def nose_detection(list_points,image_front):
    ...
    if (ratio_nose >= 3.2 and ratio_nose <= 4.0): # 평균 3.825
        ...
        nose_result = -1
        nose_percent = abs(3.6 - ratio_nose)
        #print("콧볼", ratio_nose)
...
def between_detection(list_points):
    ...
    if (between_ratio >= 3.1 and between_ratio < 3.4):
        between_result = 0
        between_percent = 0
        #print("미간 평균 ", between_ratio)

    elif (between_ratio < 3.1):
        between_result = -1
        between_percent = round(abs(3.25 - between_ratio),2)
...
try:
    # 이미지 읽어오기
    image_front_origin = cv2.imread("./front/kkk2.jpg")
    image_side_origin = cv2.imread("./side/kkk2.jpg")
    image_faceline = Image.open("./front/kkk2.jpg")
